...
- **Gamepad hotplug and input** (`patches/frt/zzzz_sdl_joystick_hotplug.patch`):
  ...
  hotplug at all. FRT watches `/dev/input` and forces SDL to re-enumerate, which
  keeps the driver and the device GUIDs — so the GameControllerDB mappings do
  not move — rather than swapping to the classic driver, which rebuilds the
  GUIDs from the device name and breaks them. The re-enumeration re-notifies
  already-connected pads, which is the price for noticing the new one. On top
  of that, SDL drops joystick axis/button events while the window is not
  focused unless background events are allowed; on a handheld, a borderless
  window or a compositor that never reports focus to SDL, the pad was detected
  but mute, so FRT now allows background joystick events.
